...
function StockTrade(props) {
  ...
  const handleBuySubmit = (event) => {
    ...
    fetch("http://stocksimulator.me:8080/api/stock/day/" + stockTicker)
      .then((response) => {
        return response.json();
      })
      // sends request to the server to buy the stocks
      .then((data) => {
        fetch("http://stocksimulator.me:8080/portfolio/buy", {
          method: "POST",
          headers: {
  ...
  const handleSellSubmit = (event) => {
    ...
    .then((data) => {
        fetch("http://stocksimulator.me:8080/portfolio/sell", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
                },
